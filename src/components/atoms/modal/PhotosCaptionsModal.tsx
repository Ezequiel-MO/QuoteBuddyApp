...
export const PhotosCaptionsModal: React.FC<PhotosCaptionsModalProps> = ({
    images,
    currentIndex,
    setCurrentIndex,
    onClose
    ...
    const handleCopy = async () => {
        ...
        if (canWriteImage) {
            try {
                const response = await fetch(imageSrc.imageUrl, { mode: 'cors' })
                ...
                if (canWriteText) {
                    try {
                        await navigator.clipboard.writeText(imageSrc.imageUrl)
    ...
    const handleDownload = () => {
        ...
        link.href = imageSrc.imageUrl
        link.download = imageSrc?.imageUrl.substring(imageSrc?.imageUrl.lastIndexOf('/') + 1)
    ...
    return (
        ...
        >
            ...
            {/* image caption */}
            <div className="absolute bottom-28 left-1/2 transform -translate-x-1/2 bg-opacity-70 text-white-0 px-6 py-2  text-xl font-semibold">
                {imageSrc.caption}
            </div>
            {/* Image Counter */}
