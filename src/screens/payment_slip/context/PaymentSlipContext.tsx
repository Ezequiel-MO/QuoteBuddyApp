...
interface IPaymentSlipContext {
	...
	setCollectionFromClient: React.Dispatch<
		React.SetStateAction<ICollectionFromClient>
	>
}
...
export const PaymentSlipProvider: FC<PaymentSlipProviderProps> = ({
	children
}) => {
	...
	const [collectionFromClient, setCollectionFromClient] =
		useState<ICollectionFromClient>({} as ICollectionFromClient)

	const { data: vendorInvoices, isLoading: isLoadingVendorInvoices } =
		useApiFetch(`vendorInvoices/project/${projectId}`, forceRefresh)
