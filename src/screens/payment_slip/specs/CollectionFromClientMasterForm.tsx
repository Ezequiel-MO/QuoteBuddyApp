...
export const CollectionFromClientMasterForm: FC<CollectionFromClientMasterFormProps> = ({ submitForm }) => {
    const {
        isUpdate,
        stateProject,
    ...
    const initialValues = getInitialValues(collectionFromClient as ICollectionFromClient)
    const validationSchema: yup.ObjectSchema<any> = VALIDATIONS.collectionFromClient
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        collectionFromClientData.clientCompanyId = stateProject?.clientCompany[0]._id ?? ""
        if (isValid) {
