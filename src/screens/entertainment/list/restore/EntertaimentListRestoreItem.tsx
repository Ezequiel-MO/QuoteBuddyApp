...
export const EntertaimentListRestoreItem: FC<EntertaimentListRestoreItemProps> = ({ item: entertainmentShow }) => {
    ...
    const handleRestore = async (entertainmentId: string) => {
        const updatedEntertainments = state.entertainments.filter(el => el._id !== entertainmentId)
        await baseAPI.patch(`entertainments/isDeleted/true/${entertainmentShow._id}`)
        dispatch({ type: 'SET_ENTERTAINMENTS', payload: updatedEntertainments })
    }

    const handleDelete = async (entertainmentId: string) => {
        const updatedEntertainments = state.entertainments.filter(el => el._id !== entertainmentId)
        await baseAPI.delete(`entertainments/isDeleted/true/${entertainmentShow._id}`)
        dispatch({ type: 'SET_ENTERTAINMENTS', payload: updatedEntertainments })
    }
    ...
    return (
        ...
        <tr className={listStyles.tr}>
            ...
            <td className={`${listStyles.td} text-red-500`}>
                {entertainmentShow?.deletedAt ? formatDate(entertainmentShow.deletedAt) : ''}
            </td>
            <td className={`${listStyles.td}`}>
                <EntertaimentDetailModal entertainment={entertainmentShow} open={openModal} setOpen={setOpenModal} />
                <MenuRestoreActions
