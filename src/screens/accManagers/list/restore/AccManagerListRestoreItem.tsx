...
export const AccManagerListRestoreItem: FC<AccManagerListItemProps> = ({ item: accManager, }) => {
    ...
    const handleRestore = async (accManagerId: string) => {
        const updatedAccManagers = state.accManagers.filter(el => el._id !== accManagerId)
    ...
    return (
        <tr className={listStyles.tr}>
            ...
            <td className={`${listStyles.td}`}>
                <AccManagerDetailModal accManager={accManager} open={openModal} setOpen={setOpenModal} />
                <MenuRestoreActions
                    item={accManager}
