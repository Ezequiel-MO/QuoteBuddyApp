...
const AudiovisualImagesContent: React.FC = () => {
	...
	const handleImageUpload = async (file: File) => {
		...
		try {
			...
			if (state.update && state.currentAudiovisual?._id) {
				const response = await baseAPI.patch(
					`audiovisuals/images/${state.currentAudiovisual._id}`,
					formData,
					{
						headers: {
	...
	const handleImageDelete = async (index: number) => {
		if (!state.currentAudiovisual?.imageContentUrl) return
	...
	const handleDragEnd = (event: any) => {
		...
		if (active.id !== over?.id) {
			const oldIndex = state.currentAudiovisual?.imageContentUrl?.findIndex(
				(url) => url === active.id
			)
			const newIndex = state.currentAudiovisual?.imageContentUrl?.findIndex(
				(url) => url === over?.id
			)
			if (
				oldIndex !== undefined &&
	...
	return (
		...
		>
			<SortableContext
				items={state.currentAudiovisual?.imageContentUrl || []}
				strategy={verticalListSortingStrategy}
			>
				<div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
					{(state.currentAudiovisual?.imageContentUrl || []).map(
						(imageSrc, index) => (
							<SortableItem
								key={imageSrc}
								id={imageSrc}
					...
					{(state.currentAudiovisual?.imageContentUrl || []).length < 12 && (
						<Thumbnail
							...
							maxFiles={
								state.currentAudiovisual?.imageContentUrl
									? 12 - state.currentAudiovisual.imageContentUrl.length
									: 12
							}
						/>
					)}
...
export default AudiovisualImagesContent
