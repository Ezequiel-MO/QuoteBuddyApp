...
export const FreelancerListRestoreItem: FC<FreelancerListRestoreItemProps> = ({
    ...
    const handleRestore = async (freelancerId: string) => {
        const updatedFreelancers = state.freelancers.filter(el => el._id !== freelancerId)
    ...
    return (
        <tr className={listStyles.tr}>
            ...
            <td className={`${listStyles.td}`}>
                <FreelancerDetailModal freelancer={freelancer} open={openModal} setOpen={setOpenModal} key={freelancer._id} />
                <MenuRestoreActions
                    item={freelancer}
                    itemType='Freelancer'
