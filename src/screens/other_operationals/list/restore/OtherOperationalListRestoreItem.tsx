...
> = ({ item: otherOperational }) => {
	...
	const handleRestore = async (otherOperationalId: string) => {
		const updatedOtherOperationals = state.otherOperationals.filter((el) => el._id !== otherOperationalId)
		await baseAPI.patch(`OtherOperationals/isDeleted/true/${otherOperationalId}`)
		dispatch({ type: 'SET_OTHEROPERATIONALS', payload: updatedOtherOperationals })
	}

	const handleDelete = async (otherOperationalId: string) => {
		const updatedOtherOperationals = state.otherOperationals.filter((el) => el._id !== otherOperationalId)
		await baseAPI.delete(`OtherOperationals/isDeleted/true/${otherOperationalId}`)
		dispatch({ type: 'SET_OTHEROPERATIONALS', payload: updatedOtherOperationals })
	}
