...
export const MeetingBreakdownRow = ({
  ...
  const handleUpdate = async (value: number, keyMeeting: string) => {
    try {
      ...
      existMeeting(dayIndex, state, typeMeeting[type] as MeetingType, idMeeting, hotelName as string)
      dispatch({
        type:"UPDATE_MEETING",
        payload:{
          value,
  ...
  return (
    <tr className='border-b border-gray-200 hover:bg-gray-100 hover:text-[#000]'>
      ...
      <td className='py-3 px-6 text-center'>
        {accounting.formatMoney(meeting[keyMeetingPrice] * meeting[keyMeetingUnit], '€')}
      </td>
    </tr>
  )
}
