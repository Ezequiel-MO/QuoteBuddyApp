...
import { UpdateTransfersItineraryPayload } from 'src/redux/features/currentProject/types'
...
export const TransferItineraryCells: FC<TransferItineraryCellsProps> = ({
	...
	const { updateTransfersItinerary, currentProject } = useCurrentProject()

	const serviceKey = transfer.selectedService as keyof ITransfer
	const serviceDescription =
		serviceDescriptions[transfer.selectedService] || transfer.selectedService

	const [originalUnitTransfer, setOriginalUnitTransfer] = useState(count)
	const [originaPriceTransfer, setOriginalPriceTransfer] = useState<number>(
		transfer[serviceKey] as number
	)
	...
	const handleUpdate = async (
		...
		try {
			...
			updateTransfersItinerary(payload)
		} catch (error: any) {
			console.log(error)
			await mySwal.fire({
				title: 'Error!',
	...
	return (
		<>
			<td>{`En Route Itinerary (Start: ${starts} , End: ${ends})`}</td>
			<td>
