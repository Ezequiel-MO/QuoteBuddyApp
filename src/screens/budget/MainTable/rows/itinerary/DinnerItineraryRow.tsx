...
import { UpdateDinnerRestaurantItineraryPayload } from 'src/redux/features/currentProject/types'
...
export const DinnerItineraryRow = ({
	...
	const { currentProject, updateDinnerRestaurantItinerary } =
		useCurrentProject()
	...
	const originalRestaurant = currentProject.schedule[
		dayIndex
	].itinerary.dinner.restaurants.find((el) => el._id === selectedEvent._id)
	...
	const handleUpdate = useCallback(
		async (newValue: number, typeValue: 'unit' | 'price') => {
			try {
				...
				const isRestaurantItinerary = existRestaurantItinerary(
					dayIndex,
					currentProject,
					'dinner',
					selectedEvent._id
				)
				if (!isRestaurantItinerary) {
					throw Error('restaurant not found')
				}
				const payload: UpdateDinnerRestaurantItineraryPayload = {
					dayIndex,
					id: selectedEvent._id,
					key: typeValue === 'unit' ? 'participants' : 'price',
					value: !newValue || newValue <= 0 ? 1 : newValue
				}
				updateDinnerRestaurantItinerary(payload)
