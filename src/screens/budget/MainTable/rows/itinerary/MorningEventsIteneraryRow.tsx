...
export const MorningEventsItenerayRow = ({
    ...
    const originalActivity = currentProject.schedule[dayIndex].itinerary.morningActivity.events.find(el => el._id === selectedEvent._id)
    ...
    const handleUpdate = async (newValue: number, typeValue: 'unit' | 'price') => {
        try {
            ...
            existActivityItinerary(dayIndex, state, "morningActivity", selectedEvent._id)
            dispatch({
    ...
    return (
        <tr className={tableRowClasses}>
            ...
            <td>
                {`En Route Itinerary, Morning Activity options`}
            </td>
            <td>
                <OptionSelect
                    options={items}
