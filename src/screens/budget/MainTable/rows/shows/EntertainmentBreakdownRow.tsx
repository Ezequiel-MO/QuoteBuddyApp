...
import { getDayIndex, existRestaurant, existEntertaiment } from "../../../helpers"
...
export const EntertainmentBreakdownRow = ({
  ...
  const handleUpdate = async (newValue: number) => {
    try {
      ...
      existRestaurant(dayIndex, state, typeMeal, selectedRestaurant._id)
      existEntertaiment(selectedRestaurant, entertaiment._id)
      const updateValue = newValue > 0 ? newValue : 0
      dispatch({
        type: "UPDATE_RESTAURANT_ENTERTAIMENT",
        payload: {
          value: updateValue,
          dayIndex,
          typeMeal,
          idRestaurant: selectedRestaurant._id,
          idEntertaiment: entertaiment._id,
          keyEntertainmentPrice
        }
      })
  ...
  return (
    <tr className="border-b border-gray-200 hover:bg-gray-100 hover:text-[#000]">
      <td align='center' className={title === "TOTAL COST" ? "text-lg font-black" : ""}>
        {title}
      </td>
      <td align='center'> </td>
      <td align='center'></td>
