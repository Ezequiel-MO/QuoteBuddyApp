...
interface ReturnProps { // esto puede ser optativo
    handleSubmit: (
        values: IProject,
        files: File[],
        endpoint: string,
...
export const useProjectSubmitForm = ({ onSuccess, onError, project }: Props) => {
    ...
    const handleSubmit = async (
        ...
        try {
            ...
            if (update) {
                const { updateTransformedData, formData } = ProjectFormData.update(values, files, open, project)
            ...
            if (!update) {
                const { formData, transformedData } = ProjectFormData.create(values, files, open)
