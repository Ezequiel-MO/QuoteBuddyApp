...
export const HotelList: FC<HotelListProps> = ({ hotels, onDelete }) => {
	...
	const [selectedHotelForMeetings, setSelectedHotelForMeetings] =
	...
	const handleOpenMeetings = (hotel: IHotel) => {
		setSelectedHotelForMeetings(hotel)
	}

	const handleCloseMeetings = (open: boolean) => {
		if (!open) {
			setSelectedHotelForMeetings(null)
		}
	}
	...
	return (
		<div className="flex flex-col items-start w-full max-w-xs p-4 rounded-lg shadow-lg bg-gray-800 text-white-0">
			...
			<div className="overflow-y-auto max-h-[500px] w-full space-y-4">
				...
				{/* Single instance of meetings modal */}
				{selectedHotelForMeetings && (
					<AddMeetingsModal
						hotel={selectedHotelForMeetings}
						isOpen={!!selectedHotelForMeetings}
						setOpen={handleCloseMeetings}
					/>
				)}
					...
					{(itemsState: IHotel[]) =>
						itemsState.map((hotel, index) => (
							<HotelCard
								...
								index={index}
								onOpenMeetings={() => handleOpenMeetings(hotel)}
							/>
						))
					}
				</DragAndDropContextProvider>
