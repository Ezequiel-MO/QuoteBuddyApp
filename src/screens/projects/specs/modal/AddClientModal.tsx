...
export const ModalAddClientForm: FC<ModalPaymentFormProps> = ({
	...
	const handleAddClientSubmit = async (dataClient: IClient) => {
		...
		try {
			...
			const update = {
				employees: [...companyEmployees, responseClient.data.data.data._id]
			}
			const responseCompany = await baseAPI.patch(
				`client_companies/${companyId}`,
				update
			) // hago un update para agregar el nuevo Client a la Company
			...
			console.log(error)
			toast.error(
				`Failed to create Client , ${error.response.data.message || ''}`,
				errorToastOptions
			)
			logger.logErrorToDatabase(
				error.response.data.message,
				`validation of the  Client  Create, in createEntity.ts`,
				'info'
			)
			throw error
		}
		...
		<>
			...
			>
				...
				<div className="text-white-0">
					<ClientMasterForm
						handleAddClient={(client) => handleAddClientSubmit(client)}
					/>
				</div>
			</ModalComponent>
		</>
